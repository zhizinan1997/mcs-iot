#!/bin/bash
# =============================================================================
# MCS-IOT 自动化备份与异地容灾脚本 (Auto-Backup & Disaster Recovery)
#
# 该脚本负责定时提取数据库核心数据，并支持本地循环存储与云端对象存储同步。
# 主要职责：
# 1. 数据库热备份：基于 pg_dump 生成压缩 SQL 包，包含所有 Hypertable 元数据。
# 2. 云端冷存储：集成 Cloudflare R2 / AWS S3 兼容协议，自动上传备份包实现异地容灾。
# 3. 智能清理：根据配置的 RETAIN_DAYS 策略自动删除过期的本地备份，防止磁盘溢出。
# 4. 任务日志：详细记录每次备份的文件名、大小、耗时及存储状态，便于溯源审计。
#
