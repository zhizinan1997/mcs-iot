<!--
  MCS-IOT 传感器管理页面 (Device/Sensor Management)

  该文件是系统最核心的设备维护界面，负责传感器的全生命周期管理。
  主要职责：
  1. 实时列表与分页：展示所有传感器的 SN、名称、状态、电池及信号，支持多维度排序。
  2. 快速编辑：支持在表格内直接修改传感器名称、监测类型、单位及报警阈值，提升效率。
  3. 校准参数配置：提供 PPM 计算公式 (Kx+B) 的深度校准接口，支持自定义温补系数。
  4. 报警阈值管理：灵活配置高/低报警限制值，联动后台报警引擎。
  5. 响应式布局：采用毛玻璃卡片设计，适配不同尺寸的显示终端。

  技术栈：Vue 3 (setup), Element Plus Select/InputNumber, Table & Pagination.
-->
<template>
