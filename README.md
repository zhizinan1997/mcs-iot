...
部署时需要设置多个密码，以下是各密码的用途说明：

#### 1. 数据库密码 (`DB_PASS`)

| 项目     | 说明                              |
| -------- | --------------------------------- |
| 用途     | PostgreSQL/TimescaleDB 数据库连接 |
| 要求     | 至少 8 位                         |
| 使用者   | Backend、Worker 服务              |
| 存储位置 | `.env` 文件                       |

#### 2. 后台管理员密码 (`ADMIN_INITIAL_PASSWORD`)
...
| 要求     | 至少 6 位                        |
| 使用者   | 管理员登录                       |
| 存储位置 | `.env` 文件 → 首次启动写入数据库 |

> 💡 **提示**: 登录后可在后台修改密码，`.env` 中的密码仅用于首次初始化。
...
| 用途     | MQTT Broker 认证                                    |
| 要求     | 至少 6 位                                           |
| 使用者   | Worker、Backend、传感器/模拟器                      |
| 存储位置 | `.env`、`mqtt_config.json`、Mosquitto `passwd` 文件 |

**MQTT 系统包含 3 个用户，共享同一密码：**

| 用户名     | 用途                  | 权限                      |
| ---------- | --------------------- | ------------------------- |
| `admin`    | 后端服务 (Backend)    | 发布 + 订阅所有主题       |
| `worker`   | 数据处理服务 (Worker) | 订阅 `mcs/+/up`、发布指令 |
| `zhizinan` | 传感器/模拟器         | 发布 `mcs/{SN}/up` 和状态 |

**配置文件位置：**

| 文件        | 路径                                             | 说明                     |
| ----------- | ------------------------------------------------ | ------------------------ |
| 环境变量    | `/opt/mcs-iot/.env`                              | 主配置，包含 `MQTT_PASS` |
| Worker 配置 | `/opt/mcs-iot/mosquitto/config/mqtt_config.json` | Worker 和 Backend 读取   |
| 模拟器配置  | `/opt/mcs-iot/scripts/mqtt_config.json`          | 模拟器脚本读取           |
| 密码文件    | Mosquitto 容器内 `/mosquitto/config/passwd`      | 加密存储                 |
| 权限配置    | Mosquitto 容器内 `/mosquitto/config/acl`         | 访问控制列表             |

### 🆕 全新部署 vs 🔄 更新部署

#### 全新部署

脚本会**提示输入**所有密码，并自动同步到各个位置：

```
用户输入 → .env → Mosquitto passwd → mqtt_config.json (x2)
```

#### 更新部署

脚本**不会再次询问密码**，而是从现有 `.env` 读取并重新同步：

1. `git pull` 拉取最新代码（会覆盖 `mqtt_config.json` 为模板）
2. 从 `.env` 读取 `MQTT_PASS`
3. 用该密码重新生成 `mqtt_config.json`
4. 用该密码同步到 Mosquitto 的 3 个用户
5. 重启服务

> ✅ **安全保证**: 更新时不会丢失或覆盖您设置的密码。
...
```bash
# 查看 MQTT 密码
grep "MQTT_PASS" /opt/mcs-iot/.env

# 查看 mqtt_config.json
cat /opt/mcs-iot/mosquitto/config/mqtt_config.json

# 查看 Mosquitto 用户列表 (密码为哈希值，无法查看明文)
docker exec mcs_mosquitto cat /mosquitto/config/passwd

# 查看 ACL 权限配置
docker exec mcs_mosquitto cat /mosquitto/config/acl
```

### ⚙️ 手动修改 MQTT 密码

如需修改 MQTT 密码，请按以下步骤操作：

```bash
# 1. 修改 .env 文件
nano /opt/mcs-iot/.env
# 修改 MQTT_PASS=新密码

# 2. 同步到 Mosquitto
docker exec mcs_mosquitto mosquitto_passwd -b /mosquitto/config/passwd admin 新密码
docker exec mcs_mosquitto mosquitto_passwd -b /mosquitto/config/passwd worker 新密码
docker exec mcs_mosquitto mosquitto_passwd -b /mosquitto/config/passwd zhizinan 新密码

# 3. 更新 mqtt_config.json (两个位置)
# 手动编辑或重新运行部署脚本
